	...
	class SupportChecker {
		...
		private class Filter implements Predicate<OperatorInfo> {
			...
			private boolean matches(Operation operation, OperatorInfo operatorInfo) {
				...
				final Map<TypeVariable<?>, Type> operationArgs = TypeUtils.getTypeArguments(operation.getClass(), Operation.class);

				final Map<TypeVariable<?>, Type> operatorArgs = TypeUtils.getTypeArguments(operatorInfo.targetType, Operation.class);
							...
							while ((currentInfo == null) || !currentInfo.hasNext()) {
								if (hierarchy.hasNext()) {
									...
									if (subgroups.containsKey(c)) {
										currentInfo = IteratorUtils.filteredIterator(subgroups.get(c).iterator(), (d) -> filter.test(d));
									}
									continue;
								}
								return false;
